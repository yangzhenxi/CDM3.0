<template>
  <div>
    <s-operator v-if="[1, 4].includes(o.app.kind)" :config="operatorConfig" />
    <s-table
      ref="table"
      :columns="columns"
      :fetchData="fetchData"
...
export default {
  ...
  methods: {
    handlePollUp: throttle(function () {
      this.$router.push({ path: `/cdm/application/${this.o.app?.id}/virtualCopy/pollUp`, query: { name: this.o.app?.name } })
    }),
