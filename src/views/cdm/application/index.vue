    ...
    <a-card :bordered="false">
      ...
      >
        ...
        <template slot="state" slot-scope="text, record">
          <a-tooltip>
            <template slot="title">
              <div v-for="item in record.instances" :key="item.id">
                <label for="name" class="colon">实例</label>
                <span>{{ deepGet(item, 'name') }}</span>
                <a-divider type="vertical" />
                <span>{{ deepGet(item, 'state') | convert('CDM_INSTANCE_STATE') }}</span>
              </div>
            </template>
        ...
        <template slot="action" slot-scope="text, record">
          <div class="table-inline-operator">
            ...
            <a-button type="primary" size="small" :disabled="record.state !== 3" @click.stop="handleBackup(record)">备份</a-button>
  ...
  data () {
    return {
        ...
        fields: [
          {
            ...
            onClick: throttle(() => {
              this.$router.push({ path: '/cdm/application/add' })
            })
          },
          {
          ...
          customCell: (record, index) => ({
            ...
            on: {
              click: e => {
                ...
                let path = `/cdm/application/${record?.id}`
                if (record?.kind === 6 || record?.kind === 3) {
                  path = `/cdm/application/${record?.id}/instances`
                }
      ...
      fetchData: async params => {
        ...
        let res = {}
        try {
          res = await getApplicationList(Object.assign(params, { type: 1 }, queryBuild(this.$refs.form.model)))
        } catch (error) {
