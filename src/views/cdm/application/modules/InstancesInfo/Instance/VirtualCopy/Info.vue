...
export default {
  ...
  methods: {
    loadData () {
      const recoverSpec = JSON.parse(deepGet(this.record, 'recoverSpec', '111'))
      const persistentVolumes = deepGet(this.record, 'persistentVolumes', [])
      const compressRatio = (persistentVolumes.reduce((pre, cur) => pre + deepGet(cur, 'compressRatio', 0), 0) / persistentVolumes.length).toFixed(2)
      this.record = Object.assign(this.record, {
        used: persistentVolumes.reduce((pre, cur) => pre + +(deepGet(cur, 'used', 0)), 0),
        ...
        pool: persistentVolumes.map(u => u?.pool),
        recoverNmae: recoverSpec?.Vmware?.recover_name
      })
