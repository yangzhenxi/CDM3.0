    ...
    <div v-for="item in data" :key="item.key">
      ...
      <detail :data="item.list" :colSpan="2" justify="end" :colon="false" >
        <template slot="time" slot-scope="text, r">
          {{ (`${deepGet(r, 'completedAt', 0)}`.substring(0, 10) - `${deepGet(r, 'backupStartAt', 0)}`.substring(0, 10)) | convert('unixDiff') }}
        </template>
      </detail>
    </div>
  </div>
...
export default {
  ...
  methods: {
    loadData () {
      ...
      const compressRatio = (snapshots.reduce((pre, cur) => pre + parseInt(deepGet(cur, 'compressRatio', 0)), 0) / snapshots.length).toFixed(2)
