...
import { deepGet, handleFetchStateErrorDone, isEmpty, throttle, toEnum, uniq } from '@/utils/util'
...
export default {
  ...
  methods: {
    handleRecover: throttle(function (record) {
      this.$router.push({ path: `/cdm/application/${record?.applicationId}/goldenCopy/recover/${record?.id}`, query: { name: record?.name, instanceId: record?.instanceId } })
    }),
    handleInfo: throttle(function (record) {
      this.$dialog(Info, {
        record: Object.assign(record, { kind: this.o.app?.kind, instanceName: this.o.instance.name })
      }, {
        width: 800,
        title: record?.name
      })
