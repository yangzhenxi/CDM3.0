...
import { enumToSelection, deepGet, transformArray } from '@/utils/util'
...
export default {
  ...
  methods: {
    async loadData () {
      this.loading = true
      const kind = +deepGet(this.record, 'kind')
      if (kind === 1 || kind === 4) {
        transformArray(this.items, { prop: 'type' }, { visible: true })
      } else {
        transformArray(this.items, { prop: 'type' }, { visible: false })
      }
