...
import { checkMssql, getAllMssqlDatabase, getMssqlDatabase } from '@/api/module/mssql'
import { FormBuilder } from '@/components'
import { useStepFormItem } from '@/core/use/form'
import { deepGet, transformArray, throttle } from '@/utils/util'
    ...
    async nextStep () {
      try {
        ...
        const databaseList = (await Promise.all(
          databases.map(u => getMssqlDatabase({
            id: this.step1.lblet?.id,
            mssqlInstance: {
              ...this.spec,
              backupDatabase: u
